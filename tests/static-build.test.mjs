...
test("GitHub Pages artifact preserves the production host and static distribution", () => {
  for (const path of ["index.html", "CNAME", ".nojekyll", "distribution/agent.txt", "favicon.svg", "social/heymanager-social.png", "product/home.jpg", "product/calendar.jpg", "art/hero-heymanager.png", "art/hero-heymanager-mobile.png", "art/assembly-computer-heymanager.png", "art/companyos-team.jpg", ...["work", "agents", "system"].map(part => `art/assembly-${part}.jpg`), "robots.txt"]) assert.ok(existsSync(`dist/${path}`), `missing ${path}`);
  assert.equal(readFileSync("dist/CNAME", "utf8").trim(), "operaos.ai");
  const html = readFileSync("dist/index.html", "utf8");
  assert.match(html, /HeyManager \| Your company\. One system\./);
  ...
  assert.match(html, /property="og:image" content="https:\/\/operaos\.ai\/social\/heymanager-social\.png"/);
...
test("FAQ provides fourteen localized native accordion items", () => {
  ...
  for (const locale of ["en", "pt-BR", "es-419"]) {
    ...
    const brandMentions = (faq.items.flat().join(" ").match(/HeyManager/g) || []).length;
    assert.equal(brandMentions, 2, `${locale} FAQ should establish HeyManager in the first item without repeating it mechanically`);
  }
  const javascript = builtJavascript();
...
test("public page uses the HeyManager identity without obsolete sales paths", () => {
  ...
  for (const text of ["HeyManager", "Agent One", "Dev One"]) assert.ok(javascript.includes(text), `missing canonical built identity: ${text}`);
